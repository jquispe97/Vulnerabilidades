...
namespace CSF.CITASWEB.WS.BatchFinesAdicionales
{
    public class ExcelMemory
    {
        ...
        private static char sepCol = '¦';
        private static char sepFilas = '¬';
        private static string color = "";
        private static List<string> listaCabecera;
        private static int _indiceInicio; //matriz

        private static List<Archivos> LstArchivos = new List<Archivos>();

        public static byte[] Exportar(string archivoXlsx, string[] hojas, List<string> objData, List<string> objCabecera, string Color = "FF193D69", int indiceColumna = 0, int indiceFila = 0, int indiceInicio = 0)
        {
            ...
            indiceY = indiceFila;
            color = Color;
            listaCabecera = objCabecera;
            _indiceInicio = indiceInicio;
            int ncolumnas, nfilas;

            for (int i = 0; i < objData.Count; i++)
            {
                data.Add(objData[i]);
                nfilas = objData[i].Split(sepFilas).Length;
                ncolumnas = objData[i].Split(sepFilas)[0].Split(sepCol).Length;
                sRango[i] = String.Format("${0}${1}", (char)(64 + ncolumnas), (nfilas - 2));//+ 1
            }
        ...
        private static byte[] crearDirectoriosArchivos()
        {
        ...
        private static string getXlStyles()
        {
            ...
            sb.Append("<font><sz val=\"11\"/><color theme=\"0\"/><name val=\"Calibri\"/><family val=\"2\"/><scheme val=\"minor\"/></font>");

            sb.Append("</fonts>");
            sb.Append("<fills count=\"3\">"); //cantidad de fill
            sb.Append("<fill><patternFill patternType=\"none\"/></fill>");
            sb.Append("<fill><patternFill patternType=\"gray125\"/></fill>");
            sb.Append("<fill><patternFill patternType=\"solid\"><fgColor rgb=\"" + color + "\"/><bgColor indexed=\"64\"/></patternFill></fill>");//para color cabecera
            sb.Append("</fills>");
            ...
            sb.Append("<xf numFmtId=\"0\" fontId=\"1\" fillId=\"2\" borderId=\"0\" xfId=\"0\" applyFont=\"1\" applyFill=\"1\"/>");//para style
            sb.Append("</cellXfs>");
        ...
        private static void getXlSheet(MemoryStream ms, int nHoja)
        {
            using (StreamWriter sw = new StreamWriter(ms))
            {
                ...
                {
                    sw.Write("<cols>");
                    string tabla = data[nHoja];

                    string[] anchos = tabla.Split(sepFilas)[1].Split(sepCol);
                    int nCampos = anchos.Length;
                    for (int j = 0; j < nCampos; j++)
                    {
                        sw.Write("<col width=\"");
                        sw.Write((int.Parse(anchos[j])).ToString());
                        sw.Write("\" min=\"");
        ...
        {
            /*Inicio la cabecera es Especifico por ahora*/
            int nCaberas = listaCabecera.Count;

            for (int i = 0; i < nCaberas; i++)
            {
                sw.Write("<row outlineLevel=\"0\" r=\"0\">", indiceY + 1);
                sw.Write("\">");
                sw.Write("<c r=\"");
                switch (i)
                {
                    case 0:
                        ...
                        sw.Write("Fecha Cita: " + listaCabecera[i]);
                        sw.Write("</t></is></c>");
                        break;
                    case 1:
                        sw.Write("A2");
                        sw.Write("\" s=\"0\" t=\"inlineStr\"><is><t>");
                        sw.Write("Clínica: " + listaCabecera[i]);
                        sw.Write("</t></is></c>");
                        break;
                    case 2:
                        sw.Write("A3");
                        sw.Write("\" s=\"0\" t=\"inlineStr\"><is><t>");
                        sw.Write("Hora Creación: " + listaCabecera[i]);
                        sw.Write("</t></is></c>");
                        break;
                    case 3:
                        sw.Write("A4");
                        sw.Write("\" s=\"0\" t=\"inlineStr\"><is><t>");
                        sw.Write("Citas sin tabla intermedia: " + listaCabecera[i]);
                        sw.Write("</t></is></c>");
                        break;
                    default:
                        break;
            ...
            string[] filas = tabla.Split(sepFilas);
            int nfilas = filas.Length;
            string[] campos = filas[2].Split(sepCol);
            int ncolumnas = campos.Length;
            string[] tipos = filas[0].Split(sepCol);
            for (int j = 0; j < ncolumnas; j++)
            {
                celda = String.Format("{0}{1}", (char)(65 + j + indiceX), indiceY + 1);
                sw.Write("<c r=\"");
                sw.Write(celda);
                sw.Write("\" s=\"1\" t=\"inlineStr\"><is><t>");
                sw.Write(campos[j]);
                sw.Write("</t></is></c>");
            }
            sw.Write("</row>");
            int cont = 0;
            for (int i = 3; i < nfilas; i++)
            {
                sw.Write("<row outlineLevel=\"0\" r=\"");
                sw.Write(cont + indiceY + 2);
                sw.Write("\">");
                campos = filas[i].Split(sepCol);
                for (int j = 0; j < ncolumnas; j++)
                {
                    celda = String.Format("{0}{1}", (char)(65 + j + indiceX), cont + indiceY + 2);
                    valor = campos[j + _indiceInicio].Replace("&", "&amp;").Replace("<", "&lt;").Replace(">", "&gt;").Replace("˜", "&tilde;").Replace("\"", "&quot;");
                    tipo = tipos[j];
                    sw.Write("<c r=\"");
                    sw.Write(celda);
                    // if (cont==0) sw.Write("\" s=\"1\"");
                    if (tipo.Contains("D")) sw.Write("\" s=\"1\""); // Date
                    else sw.Write("\" s=\"0\"");
                    if (tipo.Contains("S"))//String
                    {
        ...
        private static int buscarArchivo(string nombre)
        {
